pipeline
{
  ...
  stages
  {
    ...
    stage ('scm checkout')
    {
      steps { git 'https://github.com/Ap2109/maven-project.git' }
    }
   ...
   // CD part
    stage ('deploy the code on tomcat')
    {
      steps 
      {
        sshagent(['DEVCICD']) 
        {
          sh 'scp -o StrictHostKeyChecking=no webapp/target/webapp.war ec2-user@172.31.12.93:/usr/share/tomcat/webapps'
        }
      }
    }
  }
